...
$restore = function ($id) {
    ...
    $pelanggan->restore();
    session()->flash('message', 'Pelanggan berhasil dipulihkan!');
};

with(function () {
    return [
        'trashedPelanggans' => Pelanggan::onlyTrashed()->with('user')->orderBy('deleted_at', 'desc')->get(),
    ];
});
    ...
    <div class="py-6">
        <div class="mb-8">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Arsip Pelanggan</h1>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-300 font-medium">Data pelanggan yang telah dihapus</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-inputBorder overflow-hidden">
            @forelse($trashedPelanggans as $p)
                <div class="flex items-center justify-between p-5 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="text-sm font-bold text-textDark">{{ $p->nama }}</h4>
                            <p class="text-xs text-textGray mt-0.5">{{ $p->email }} &middot; Dihapus {{ $p->deleted_at->diffForHumans() }}</p>
                        </div>
                    </div>
            ...
            @empty
                <div class="p-12 text-center">
                    ...
                    <p class="mt-4 text-sm font-medium text-gray-500">Tidak ada pelanggan yang dihapus.</p>
                </div>
            @endforelse
        </div>
    </div>
