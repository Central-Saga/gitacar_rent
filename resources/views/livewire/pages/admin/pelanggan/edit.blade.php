...
mount(function (Pelanggan $pelanggan) {
    if (!$pelanggan->user) {
        session()->flash('error', 'Akun user untuk pelanggan ini tidak ditemukan.');
    ...
    $this->foto_ktp_lama = $pelanggan->foto_ktp;
});

$save = function() {
    $this->validate([
        'nama' => 'required|string|max:255',
        'email' => 'required|string|email|max:255|unique:users,email,'.$this->pelanggan->user_id,
    ...
    $fotoPath = $this->foto_ktp_lama;
    if ($this->foto_ktp) {
        if ($this->foto_ktp_lama) {
            ...
            <div class="mb-10">
                <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600 font-medium">Perbarui data pelanggan dan informasi akun.</p>
                    </div>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-8">
                <form wire:submit="save" class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    @elseif ($foto_ktp_lama)
                                        <div class="mt-4">
                                            <p class="text-sm font-medium text-gray-700 mb-2">Foto KTP Saat Ini:</p>
                                            <img src="{{ Storage::url($foto_ktp_lama) }}" class="h-32 rounded-lg object-cover border border-gray-200">
                                        </div>
                                    @endif
                    ...
                    <div x-data="{ open: true }" class="bg-gray-50 border border-inputBorder rounded-2xl p-6 transition-all duration-300">
                        ...
                        <div x-show="open" x-collapse class="mt-6">
                            <p class="text-sm font-bold text-amber-600 bg-amber-50 p-4 rounded-xl border border-amber-200 mb-6">Biarkan kosong jika tidak ingin mengubah password.</p>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
