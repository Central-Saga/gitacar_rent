                        ...
                        <div x-data="{ expanded: false }" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-all duration-300 hover:shadow-md">
                            ...
                            <div x-show="expanded" x-collapse>
                                <div class="px-5 sm:px-6 pb-6 pt-2 border-t border-gray-100 bg-gray-50/50">
                                            ...
                                            <ul class="space-y-2 text-sm">
                                                <li class="flex justify-between">
                                                    @php
                                                        ...
                                                        $hargaSewa = $pesanan->harga_sewa ?? $pesanan->harga_per_hari;
                                                        $hargaPerHari = $pesanan->harga_per_hari;
                                                    @endphp
                                                    <span class="text-gray-600">Berdasarkan Harga</span>
                                                    <span class="font-bold capitalize">{{ $tipe }}</span>
                                                </li>
                                                ...
                                                @if($tipe === 'bulanan')
                                                    ...
                                                    <li class="flex justify-between">
                                                        ...
                                                        <span class="font-medium text-gray-900">Rp {{ number_format($months * $hargaSewa, 0, ',', '.') }}</span>
                                                    </li>
                                                    @if($remainingDays > 0)
                                                        <li class="flex justify-between">
                                                            <span class="text-gray-600">{{ $remainingDays }} Hari x Rp {{ number_format($hargaPerHari, 0, ',', '.') }}</span>
                                                            <span class="font-medium text-gray-900">Rp {{ number_format($remainingDays * $hargaPerHari, 0, ',', '.') }}</span>
                                                        </li>
                                                    @endif
                                                @elseif($tipe === 'mingguan')
                                                    ...
                                                    <li class="flex justify-between">
                                                        ...
                                                        <span class="font-medium text-gray-900">Rp {{ number_format($weeks * $hargaSewa, 0, ',', '.') }}</span>
                                                    </li>
                                                    @if($remainingDays > 0)
                                                        <li class="flex justify-between">
                                                            <span class="text-gray-600">{{ $remainingDays }} Hari x Rp {{ number_format($hargaPerHari, 0, ',', '.') }}</span>
                                                ...
                                                @else
                                                    <li class="flex justify-between">
                                                        ...
                                                        <span class="font-medium text-gray-900">Rp {{ number_format($durasi * $hargaSewa, 0, ',', '.') }}</span>
                                                    </li>
                                                @endif

                                                <li class="flex justify-between pt-2 border-t border-gray-100">
                                                    <span class="text-gray-600 font-semibold">Subtotal</span>
                                                    <span class="font-medium text-gray-900">Rp {{ number_format($pesanan->total_harga + $pesanan->total_diskon, 0, ',', '.') }}</span>
                                                </li>
                                    ...
                                    @if($pesanan->lokasi_url || $pesanan->lokasi_deskripsi)
                                    <div class="mt-4 p-4 bg-blue-50 text-blue-800 rounded-lg border border-blue-100 space-y-2">
                                        <strong class="flex items-center gap-1.5 text-sm">
                                        ...
                                        @if($pesanan->lokasi_url)
                                        <div>
                                            <p class="text-xs font-medium opacity-80 mb-0.5">Google Maps</p>
                                            <a href="{{ $pesanan->lokasi_url }}" target="_blank" rel="noopener noreferrer" class="text-blue-700 hover:text-blue-900 font-semibold inline-flex items-center gap-1 break-all">
                                                {{ $pesanan->lokasi_url }}
                                        ...
                                        @endif
                                        @if($pesanan->lokasi_deskripsi)
                                        <div>
                                            <p class="text-xs font-medium opacity-80 mb-0.5">Deskripsi Lokasi</p>
                                            <p class="text-sm">{{ $pesanan->lokasi_deskripsi }}</p>
                                        </div>
                                        @endif
                                    </div>
                                    @endif
