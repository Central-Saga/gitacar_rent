...
on([
    'delete-pelanggan' => function ($id) {
        $pelanggan = Pelanggan::find($id);
        if ($pelanggan) {
            $pelanggan->delete();
            $this->dispatch('swal:toast', title: 'Pelanggan berhasil dihapus!', icon: 'success');
        }
    }
]);
                                        ...
                                        <div class="flex-shrink-0 h-12 w-12">
                                            @if($pelanggan->foto_ktp)
                                                <img class="h-12 w-12 rounded-2xl object-cover border border-gray-200"
                                                    src="{{ Storage::url($pelanggan->foto_ktp) }}" alt="{{ $pelanggan->nama }}">
                                            @else
                                                <div
                                                    class="h-12 w-12 rounded-2xl bg-gray-50 text-gray-800 flex items-center justify-center border border-gray-200">
                                                    <span
                                                        class="font-bold text-lg">{{ strtoupper(substr($pelanggan->nama ?? 'P', 0, 1)) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                        <div class="ml-5">
                                ...
                                <td class="px-8 py-6 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex space-x-3">
                                        ...
                                        <button wire:click="$dispatch('swal:confirm', {
                                                                        ...
                                                                        text: 'Yakin ingin menghapus pelanggan {{ $pelanggan->nama }}? Data dapat dipulihkan dari arsip.',
                                                                        icon: 'warning',
                                                                        method: 'delete-pelanggan',
                                                                        id: {{ $pelanggan->id }}
                                                                    })"
