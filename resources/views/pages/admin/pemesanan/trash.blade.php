...
layout('layouts.app');
title('Arsip Pemesanan');

$restore = function ($id) {
    $pemesanan = Pemesanan::onlyTrashed()->findOrFail($id);
    $pemesanan->restore();
    session()->flash('message', 'Pemesanan berhasil dipulihkan!');
};

with(function () {
    return [
        'trashedPemesanans' => Pemesanan::onlyTrashed()
            ->with(['pelanggan', 'kendaraanUnit.kendaraan'])
    ...
    <div class="py-6">
        <div class="mb-8">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Arsip Pemesanan</h1>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-300 font-medium">Data pemesanan yang telah dihapus</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-inputBorder overflow-hidden">
            @forelse($trashedPemesanans as $p)
                <div class="flex items-center justify-between p-5 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="text-sm font-bold text-textDark">#{{ $p->id }} — {{ $p->pelanggan?->nama ?? 'Unknown' }}</h4>
                            <p class="text-xs text-textGray mt-0.5">
                                {{ $p->kendaraanUnit?->kendaraan?->nama_kendaraan ?? '-' }} ({{ $p->kendaraanUnit?->nomor_plat ?? '-' }})
                                &middot; Dihapus {{ $p->deleted_at->diffForHumans() }}
                            </p>
                        </div>
                    </div>
            ...
            @empty
                <div class="p-12 text-center">
                    ...
                    <p class="mt-4 text-sm font-medium text-gray-500">Tidak ada pemesanan yang dihapus.</p>
                </div>
            @endforelse
        </div>
    </div>
